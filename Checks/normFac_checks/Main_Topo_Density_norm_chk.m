%  To check this: we normalize the intralayer network, but not the 
%  interlayer connections. Thus interlayer connection strength increases to
%  compensate for the scaling due to density. 
...
for inet2 = 1:4   % second layer loop 
    ...
    mkdir(['norm_check/trial=',num2str(itrial),'/inet1=',num2str(inet1),'_inet2=',num2str(inet2)])
    dirname = ['norm_check/trial=',num2str(itrial),'/inet1=',num2str(inet1),'_inet2=',num2str(inet2)]; 
...
for idens = 1:size(loop_vec,2)
    ...
    elseif inet2 ==4
        ...
        % reset interlayer connection strength 
        Duplex( N+1:end,1:N) =eye(N); 
