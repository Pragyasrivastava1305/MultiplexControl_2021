function [state,costate,E] = OneModeSol_funV1(xi, mu,Clm, T, nt, XF)
...
% @pragya, Fall 2019
% @pragya, Spring 2020: corrected some mistakes
% INPUTS ------------------------------------------------------------------
...
if (xi ~= mu) && (xi ~= 0) && (mu ~= 0)
    ...
    den1 = (-1 + exp(2*T*mu))*(-1 + exp(2*T*xi))*mu^2 - ...
            2*(1 + exp(2*T*mu) + exp(2*T*xi) - 4*exp(T*(mu + xi)) + exp(2*T*(mu + xi)))*mu*xi ...
             + (-1 + exp(2*T*mu))*(-1 + exp(2*T*xi))*xi^2; 
      
    num1 = 4*Clm*exp(T*(mu + xi)) * ( cosh(T*mu) * ( xi*sinh(tarray*mu) - mu*sinh(tarray*xi) ) + ...
    ...
    num2 = 4*exp(T*(mu + xi)) * ( mu*(-cosh(T*mu) + cosh(T*xi) + sinh(T*mu)) * ...
